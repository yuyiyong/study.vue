<!--
 * @LastEditors: yyy_dui
 * @LastEditTime: 2021-03-10 17:26:27
...
<template>
  <div>
    ...
    <p>v-for</p>
    <ol>
      <li v-bind:key="key" v-for="(item, key) in todos">{{ item.text + key}}</li>
    </ol>
    <hr />
    <p>v-on 添加监听器</p>
    <p>{{ p2Message }}</p>
...
export default {
  ...
  data () {
    return {
      ...
      newText: '',
      viewWindow: '',
      text_abbreviation: '!-- 完整语法 -- v-on:click="doSomething"..!-- 缩写 -- @click="doSomething..-- 动态参数的缩写 (2.6.0+) -- @[event]="doSomething" '
    }
  },
  components: {
    SubComponent
